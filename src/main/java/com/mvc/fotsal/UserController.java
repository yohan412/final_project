...
public class UserController {
	...
	public String userNaverLoginPro(Model model,@RequestParam Map<String,Object> paramMap, @RequestParam String code, @RequestParam String state,HttpSession session) throws SQLException, Exception {
		...
		}else if(naverConnectionCheck.get("USER_CONCHK").equals("NAVER") && naverConnectionCheck.get("USER_EMAIL") != null) { //이메일 가입 되어있고 네이버 연동 안되어 있을시
			biz.setNaverConnection(apiJson);
			UserDto dto = biz.userNaverLoginPro(apiJson);
			session.setAttribute("login", dto);
		}else if(naverConnectionCheck.get("USER_CONCHK").equals("KAKAO") && naverConnectionCheck.get("USER_EMAIL") != null) {
    ...
    public String kakaoLogin(Model model, @RequestParam("code") String code, HttpSession session) {
        ...
        } else if(kakaoConnectionCheck.get("USER_CONCHK").equals("KAKAO") && kakaoConnectionCheck.get("USER_EMAIL") != null) { //이메일 가입 되어있고 카카오 연동 안되어 있을시
        	biz.setKakaoConnection(loginApi);
        	UserDto dto = biz.userKakaoLoginPro(loginApi);
        	session.setAttribute("login", dto);
        }else if(kakaoConnectionCheck.get("USER_CONCHK").equals("NAVER") && kakaoConnectionCheck.get("USER_EMAIL") != null) {
